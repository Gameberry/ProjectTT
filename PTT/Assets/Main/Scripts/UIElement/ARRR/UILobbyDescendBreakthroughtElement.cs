...
{
    public class UILobbyDescendBreakthroughtElement : MonoBehaviour
    {
        [SerializeField]
        private Image _guidebreak3BG;
        ...
        private DescendBreakthroughData _currentSynergyEffectData;
        bool isNew = false;
        ...
        //------------------------------------------------------------------------------------
        public void SetSynergyEffectData(DescendBreakthroughData synergyEffectData)
        {
            _currentSynergyEffectData = synergyEffectData;

            if (_synergyEffectIcon != null)
            {
                _synergyEffectIcon.sprite = Managers.DescendManager.Instance.GetSynergyBreakthroughSprite(synergyEffectData);
            }

            bool geted = Managers.DescendManager.Instance.IsGetedBreakthrough(synergyEffectData);
            ...
            isNew = Managers.DescendManager.Instance.IsNewSynergyBreakthroughIcon(synergyEffectData);
        ...
        private void OnClick()
        {
            ...
            if (isNew == true)
            {
                Managers.DescendManager.Instance.RemoveNewIconBreakthroughSynergy(_currentSynergyEffectData);
            }
            ...
            Contents.GlobalContent.ShowGoodsDescPopup(V2Enum_Goods.DescendBreak, _currentSynergyEffectData.Index, 0);
        }
        //------------------------------------------------------------------------------------
    }
}