...
namespace GameBerry
{
    public static class Util
    {
        ...
        public static void ScrollViewSnapToItem(ScrollRect scrollView, RectTransform contentTrans, RectTransform target)
        {
            ...
            if (scrollView.vertical == false)
                point.y = 0f;

            if (scrollView.horizontal == false)
                point.x = 0f;

            contentTrans.anchoredPosition = point;
        }

        public static void ScrollViewSnapToItem(ScrollRect scrollView, RectTransform contentTrans, RectTransform target, Vector2 offset)
        {
            Vector2 point = (Vector2)scrollView.transform.InverseTransformPoint(contentTrans.position)
                - (Vector2)scrollView.transform.InverseTransformPoint(target.position)
                - new Vector2(0f, target.sizeDelta.y / 2f);
            ...
            Vector3 localpos = contentTrans.localPosition;
            localpos.x += offset.x;
            localpos.y += offset.y;
        ...
        public static string GetSpriteVariationName(string spritename)
        {
            string applyspritename = string.Empty;
            string[] spritenamearr = spritename.Split('_');
            bool SearckFirstNum = false;


            for (int l = 0; l < spritenamearr.Length; ++l)
            {
                string applystring = string.Empty;

                if (SearckFirstNum == false)
                {
                    int num = 0;
                    if (int.TryParse(spritenamearr[l], out num))
                    {
                        SearckFirstNum = true;
                        applystring = "{0}";
                    }
                    else
                        applystring = spritenamearr[l];
                }
                else
                {
                    applystring = spritenamearr[l];
                ...
                applyspritename += applystring;
                if (l < spritenamearr.Length - 1)
                    applyspritename += "_";
            }

            return applyspritename;
        }
...
//        }
        ...
        public static string GetAlphabetNumber(double value)
        {
            double valueToConvert = value;
            bool isnegative = false;
            if (valueToConvert < 0.0d)
            {
                isnegative = true;
                valueToConvert *= -1.0d;
            }
            ...
            string converted;
            double log10 = System.Math.Log10(valueToConvert);
            if (log10 >= 81) { converted = ((valueToConvert / 1e81)).ToString("N3") + "Z"; }
            else if (log10 >= 78) { converted = ((valueToConvert / 1e78)).ToString("N3") + "Y"; }
            else if (log10 >= 75) { converted = ((valueToConvert / 1e75)).ToString("N3") + "X"; }
            else if (log10 >= 72) { converted = ((valueToConvert / 1e72)).ToString("N3") + "W"; }
            else if (log10 >= 69) { converted = ((valueToConvert / 1e69)).ToString("N3") + "V"; }
            else if (log10 >= 66) { converted = ((valueToConvert / 1e66)).ToString("N3") + "U"; }
            else if (log10 >= 63) { converted = ((valueToConvert / 1e63)).ToString("N3") + "T"; }
            else if (log10 >= 60) { converted = ((valueToConvert / 1e60)).ToString("N3") + "S"; }
            else if (log10 >= 57) { converted = ((valueToConvert / 1e57)).ToString("N3") + "R"; }
            else if (log10 >= 54) { converted = ((valueToConvert / 1e54)).ToString("N3") + "Q"; }
            else if (log10 >= 51) { converted = ((valueToConvert / 1e51)).ToString("N3") + "P"; }
            else if (log10 >= 48) { converted = ((valueToConvert / 1e48)).ToString("N3") + "O"; }
            else if (log10 >= 45) { converted = ((valueToConvert / 1e45)).ToString("N3") + "N"; }
            else if (log10 >= 42) { converted = ((valueToConvert / 1e42)).ToString("N3") + "M"; }
            else if (log10 >= 39) { converted = ((valueToConvert / 1e39)).ToString("N3") + "L"; }
            else if (log10 >= 36) { converted = ((valueToConvert / 1e36)).ToString("N3") + "K"; }
            else if (log10 >= 33) { converted = ((valueToConvert / 1e33)).ToString("N3") + "J"; }
            else if (log10 >= 30) { converted = ((valueToConvert / 1e30)).ToString("N3") + "I"; }
            else if (log10 >= 27) { converted = ((valueToConvert / 1e27)).ToString("N3") + "H"; }
            else if (log10 >= 24) { converted = ((valueToConvert / 1e24)).ToString("N3") + "G"; }
            else if (log10 >= 21) { converted = ((valueToConvert / 1e21)).ToString("N3") + "F"; }
            else if (log10 >= 18) { converted = ((valueToConvert / 1e18)).ToString("N3") + "E"; }
            else if (log10 >= 15) { converted = ((valueToConvert / 1e15)).ToString("N3") + "D"; }
            else if (log10 >= 12) { converted = ((valueToConvert / 1e12)).ToString("N3") + "C"; }
            else if (log10 >= 9) { converted = ((valueToConvert / 1e9)).ToString("N3") + "B"; }
            else if (log10 >= 6) { converted = ((valueToConvert / 1e6)).ToString("N3") + "A"; }
            else
            {
            ...
            if (isnegative == true)
                return string.Format("-{0}", converted);
            else
                return converted;
        }
        ...
        public static void ConvertRewardArrStringInList_int(string str, ref List<int> intlist)
        { 
            string convertstr = str.Replace("[", string.Empty).Replace("]", string.Empty);

            string[] convertstrarr = convertstr.Split(',');
            for (int i = 0; i < convertstrarr.Length; ++i)
            {
                if (string.IsNullOrEmpty(convertstrarr[i]) == true)
                    continue;

                intlist.Add(convertstrarr[i].ToInt());
            }
        }

        public static bool CheckCharacterCount(string checkstr, int min, int max, bool shownotice = true)
        {
            if (checkstr.Length < min || checkstr.Length > max)
            {
                Contents.GlobalContent.ShowGlobalNotice(Managers.LocalStringManager.Instance.GetLocalString("PlayerNick_Message_CharacterCount"));
                return false;
            }
        ...
        public static bool ContainSpecialCharacter(string checkstr, bool shownotice = true)
        {
            string idChecker = System.Text.RegularExpressions.Regex.Replace(checkstr, string.Format(@"{0}", Managers.SceneManager.Instance.SpacialCharRegex), string.Empty, System.Text.RegularExpressions.RegexOptions.Singleline);

            if (checkstr != idChecker)
            {
                if (shownotice == true)
                    Contents.GlobalContent.ShowGlobalNotice(Managers.LocalStringManager.Instance.GetLocalString("PlayerNick_Message_SpecialCharacter"));
                return true;
            }
        ...
        public static string ConvertListToString(List<double> listdata)
        {
            if (listdata == null)
                return string.Empty;
